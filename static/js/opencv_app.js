...
videoStream.addEventListener('mousedown', function(e) {
    ...
    if (measureMode === 'marker' || measureMode === 'pickup') {
...
videoStream.addEventListener('mouseup', function(e) {
    ...
    if (measureMode === 'marker' || measureMode === 'pickup') {
        ...
        // Radius = nửa kích thước trung bình của vùng vẽ
        const radius = Math.round(((x2 - x1) + (y2 - y1)) / 4);
        handleBinPosition(cx, cy, radius);
...
function startMeasure(mode) {
    ...
    ind.classList.add('active', mode === 'bin' ? 'mode-bin' : 'mode-measure');

    const hints = {
        bin: 'Kéo vùng để đặt vị trí <strong>BIN</strong>',
...
function handleBinPosition(x, y, radius) {
    document.getElementById('bin-x').value = x;
    document.getElementById('bin-y').value = y;
    if (radius && radius > 10) {
        document.getElementById('bin-r').value = radius;
    }
    saveConfigSilent();
    cancelMeasure();
}
...
async function saveConfigSilent() {
    ...
    const cfg = {
        ...
        container_x: +document.getElementById('bin-x').value,
        container_y: +document.getElementById('bin-y').value,
        container_margin: +document.getElementById('bin-r').value,
...
async function loadConfig() {
    try {
        ...
        document.getElementById('bin-x').value = data.container_pos[0];
        document.getElementById('bin-y').value = data.container_pos[1];
        document.getElementById('bin-r').value = data.container_margin || 120;
...
async function updateStatus() {
    try {
        ...
        greenEl.className = 'val ' + (data.green_detected ? 'ok' : '');
        ...
        const greenDistEl = document.getElementById('stat-green-dist');
        greenDistEl.textContent = data.dist_to_green > 0 ? data.dist_to_green + 'px' : '--';
        const pickupDist = data.pickup_distance || 80;
        greenDistEl.className = 'val ' + (data.dist_to_green > 0 && data.dist_to_green <= pickupDist ? 'ok' : (data.dist_to_green > 0 ? 'warn' : ''));

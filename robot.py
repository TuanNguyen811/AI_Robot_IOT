...
# ================== ROBOT CONFIG ==================
ROBOT_IP = "192.168.1.23"  # Thay bằng IP thực của ESP8266
ROBOT_PW = "5613"
...
HEARTBEAT_INTERVAL = 10.0  # Gửi lại lệnh sau 3 giây nếu không đổi
...
def send_robot_command(cmd, force=False):
    ...
    # Không gửi nếu lệnh giống lệnh trước VÀ chưa hết thời gian heartbeat (trừ khi force)
    if not force and cmd == _last_sent_cmd:
        if now - _last_sent_time < HEARTBEAT_INTERVAL:
            return
        # Hết thời gian heartbeat -> gửi lại
...
def decide_move_to_target(target_pos, heading_info, frame_w, frame_h):
    ...
    if heading_info is not None:
        ...
        if vec_len < 50:  # Đủ gần target
            return "STOP"
        
        vec_x /= vec_len
        vec_y /= vec_len
        
        # Dot và cross product
